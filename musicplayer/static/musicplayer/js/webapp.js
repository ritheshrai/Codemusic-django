  ...
  if(xhttp.status==200){
    ...
    if(k.Result=='false'){
      ...
      for (i = 0; i < len; i++) 
      {
        ...
        god=god+good;
        songarray[i]=k[i].fields.musicID
...
function nextsong(){
  pauseSong()
  if(songarray.length-1>-1)
  {
    if(songarray.length-1>cursong){
      djangosong(songarray[cursong+1]);
    }
    else if(songarray.length-1==cursong){
      djangosong(songarray[0]);
    }
  }}
  function prevsong(){
    pauseSong()
    if(songarray.length-1>0&&songarray.length>cursong){
      djangosong(songarray[cursong-1])
    }
    else if(cursong==0){
      djangosong(songarray[songarray.length-1])
    }
  }
